...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Pct labels (report-style: whole numbers)
  const mp = (n, t) => { const p = pctRound(n, t); return `<span class="kpi-pct ${p>50?'up':'down'}">${p}%</span>`; };
  setEl('kpi-killed-pct',   pct(q4.killed, q4.total));
  setEl('kpi-injured-pct',  pct(q4.injured, q4.total));
  setEl('kpi-abducted-pct', pct(q4.abducted, q4.total));
  setEl('kpi-crsv-pct',     pct(q4.crsv, q4.total));

  // Q3 vs Q4 change (report-style: whole numbers)
  const chgHtml = (v, p) => {
    if (!p) return '';
    const d = Math.round((v-p)/p*100);
  ...
  if (barsEl) {
    ...
    barsEl.innerHTML = vItems.map(item => {
      const p = pctRound(item.val, q4.total);
      return `
  ...
  if (qoqEl) {
    ...
    qoqEl.innerHTML =     items.map(item => {
      const diff = item.prev ? Math.round((item.cur - item.prev)/item.prev*100) : 0;
      const sign = diff > 0 ? '+' : '';
  ...
  const areaTraces = vKeys.map((v, i) => ({
    type: 'scatter',
    mode: 'lines',
    stackgroup: 'one',
    name: vLabels[i],
  ...
  const monthlyTraces = vKeys.map((v, i) => ({
    type: 'scatter',
    mode: 'lines+markers',
    name: vLabels[i],
    x: mLabels,
  ...
  const perpData = PERPS.map(p => ({
    name: p, val: d.q4_by_perpetrator[p]?.total || 0, col: pColor(p)
  }));
  ...
  const stVioTraces = vKeys.map((v,i) => ({
    type: 'bar',
    name: vLabels[i],
    y: sts,
  ...
  Plotly.newPlot('chart-state-bar', stVioTraces, {
    ...
    xaxis: { title: 'Victims', gridcolor: 'rgba(0,158,219,0.12)' },
  ...
  const perpNames = ['Community-based Militias','Conventional Parties','Unidentified/Opportunistic'];
  ...
  const chg = Math.round((q4.total - q3Total)/q3Total*100);
  ...
  document.getElementById('insight-main').innerHTML = `
    <ul class="insight-list">
      ...
      <li>Killing remains the most prevalent form of harm at <strong class="highlight">${pctRound(q4.killed, q4.total)}%</strong> of all victims, followed by injury (${pctRound(q4.injured, q4.total)}%).</li>
      <li><strong class="highlight">${topState}</strong> is the most affected state with <strong class="highlight">${fmt(topStateData.total || 0)} victims</strong> — ${pctRound(topStateData.total||0, q4.total)}% of the national total.</li>
      <li>Men comprise <strong class="highlight">${pctRound(g.male, q4.total)}%</strong> of all victims, while women and children together represent <strong class="highlight">${pctRound(g.female+g.boys+g.girls, q4.total)}%</strong>.</li>
    </ul>`;

  const topPerp = PERPS.reduce((a,b) => (d.q4_by_perpetrator[a]?.total||0) > (d.q4_by_perpetrator[b]?.total||0) ? a : b);
  const topPerpPct = pctRound(d.q4_by_perpetrator[topPerp]?.total||0, q4.total);

  document.getElementById('insight-secondary').innerHTML = `
    <ul class="insight-list">
      <li><strong class="highlight">${topPerp}</strong> are responsible for <strong class="highlight">${topPerpPct}%</strong> of all Q4 civilian casualties, continuing as the dominant perpetrator group.</li>
      <li>CRSV accounted for <strong class="highlight">${fmt(q4.crsv)} victims</strong> (${pctRound(q4.crsv, q4.total)}%), with women and girls disproportionately affected.</li>
